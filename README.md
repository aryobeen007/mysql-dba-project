# MySQL DBA Project

**Author:** Naseer Aryobee  
**Portfolio:** [nasaryobee.com](https://nasaryobee.com)  
**GitHub:** [github.com/aryobeen007/mysql-dba-project](https://github.com/aryobeen007/mysql-dba-project)

This is an end-to-end MySQL database administration project built on publicly available environmental and public health datasets exploring the relationship between Concentrated Animal Feeding Operations (CAFOs), water contamination, and community health outcomes.

## Phases

| Phase | Topic | Status |
|-------|-------|--------|
| 1 | Data Sourcing and Documentation | 🔄 In Progress |
| 2 | Installation, Schema Design, and Data Loading | ⏳ Pending |
| 3 | Baseline Measurement and Diagnostics | ⏳ Pending |
| 4 | Performance Optimization | ⏳ Pending |
| 5 | Backup and Recovery | ⏳ Pending |
| 6 | Security and User Management | ⏳ Pending |
| 7 | Portfolio Integration | ⏳ Pending |
